print_rootograms = function() {
  ...
  for (DV in outcomes$code) {
    ...
    print(paste('rootogram ', DV))
  	...
  	xmax = min(rootogram.list[[DV]][which(frq.pred >= 5), max(count)],
  						 rootogram.list[[DV]][which(frq.obs  >= 5), max(count)])
  ...
  rootogram.dt %>%
    	...
    	geom_rect(aes(xmin = rect.x - rect.width/2,
  		...
  		geom_point(mapping = aes(x = count, y = point.y)) +
  ...
  rootogram.filename = paste0(outputPath.results, 'rootograms_', subcohort, timeNow(),'.png')

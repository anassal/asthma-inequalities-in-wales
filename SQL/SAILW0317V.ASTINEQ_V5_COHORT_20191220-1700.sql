...
CREATE OR REPLACE VARIABLE SAILW0317V.ASTINEQ_V5_COHORT_STUDY_STARTDATE DATE DEFAULT '2010-01-01';  -- NULL
CREATE OR REPLACE VARIABLE SAILW0317V.ASTINEQ_V5_COHORT_STUDY_ENDDATE DATE DEFAULT '2014-12-31';  -- NULL

ALTER TABLE SAILW0317V.ASTINEQ_V5_COHORT 
	 ADD 	ASTHMA_GP_VISITS DECIMAL(12,6) DEFAULT 0
 ADD 	SABA DECIMAL(12,6) DEFAULT 0
 ADD 	ICS DECIMAL(12,6) DEFAULT 0
 ADD 	ASTHMA_SPIROMETRY DECIMAL(12,6) DEFAULT 0
 ADD 	ASTHMA_PEFR DECIMAL(12,6) DEFAULT 0
 ...
 ADD 	PEDW_ASTHMA_ELECT SMALLINT DEFAULT 0
 ADD 	PEDW_ASTHMA_MATRN SMALLINT DEFAULT 0
 ADD 	PEDW_ASTHMA_OTHER SMALLINT DEFAULT 0
 ADD 	EDDS_ASTHMA SMALLINT DEFAULT 0;
		 ...
		 MERGE INTO			SAILW0317V.ASTINEQ_V5_COHORT A
		 USING				(
		 

		 SELECT 		A1.ALF_PE,
		 count(DISTINCT G.EVENT_DT) AS	ASTHMA_GP_VISITS
		 FROM		SAILW0317V.ASTINEQ_V5_COHORT  A1
		 	   	   JOIN	SAILW0317V.ASTINEQ_V5_COHORT1_WLGP_SUBSET  G
		 	   	   ...
		 	   	   AND		(
						...
						AND 		(G.EVENT_DT >= SAILW0317V.ASTINEQ_V5_COHORT_STUDY_STARTDATE AND G.EVENT_DT < SAILW0317V.ASTINEQ_V5_COHORT_STUDY_ENDDATE)
		 ...
		 SELECT 		A1.ALF_PE,
		 count(DISTINCT G.EVENT_DT) AS	ASTHMA_SPIROMETRY
		 FROM		SAILW0317V.ASTINEQ_V5_COHORT  A1
		 	   	   JOIN	SAILW0317V.ASTINEQ_V5_COHORT1_WLGP_SUBSET  G
		 	   	   ON		A1.ALF_PE = G.ALF_PE
		 	   	   AND		(
										G.EVENT_CD IN ('33G1.', '33H1.', '33I1.', '33J1.', '33K1.', '663J.', '745D4', '8HRC.')
									)
						AND 		(G.EVENT_DT >= SAILW0317V.ASTINEQ_V5_COHORT_STUDY_STARTDATE AND G.EVENT_DT < SAILW0317V.ASTINEQ_V5_COHORT_STUDY_ENDDATE)
						GROUP BY A1.ALF_PE, ASTHMA_SPIROMETRY
		 ...
		 WHEN MATCHED THEN	UPDATE SET A.ASTHMA_SPIROMETRY = B.ASTHMA_SPIROMETRY;
		 ...
		 SELECT 		A1.ALF_PE,
		 count(DISTINCT G.EVENT_DT) AS	ASTHMA_PEFR
		 FROM		SAILW0317V.ASTINEQ_V5_COHORT  A1
		 	   	   JOIN	SAILW0317V.ASTINEQ_V5_COHORT1_WLGP_SUBSET  G
		 	   	   ON		A1.ALF_PE = G.ALF_PE
		 	   	   AND		(
										G.EVENT_CD IN ('33950', '339A.', '339B.', '339c.', '339d.', '339g.', '339n.', '66YX.', '66YY.', '66Yc.')
									)
						AND 		(G.EVENT_DT >= SAILW0317V.ASTINEQ_V5_COHORT_STUDY_STARTDATE AND G.EVENT_DT < SAILW0317V.ASTINEQ_V5_COHORT_STUDY_ENDDATE)
						GROUP BY A1.ALF_PE, ASTHMA_PEFR
		 ...
		 WHEN MATCHED THEN	UPDATE SET A.ASTHMA_PEFR = B.ASTHMA_PEFR;
		 ...
		 -- ASTHMA_LOS
		 -- Total LOS for asthma 2010-2014
		 ...
		 SELECT 		ALF_PE,
		 NULL AS	ASTHMA_LOS
		 FROM		SAILW0317V.ASTINEQ_V5_COHORT
		 ...
		 WHEN MATCHED THEN	UPDATE SET A.PEDW_ASTHMA_ELECT = B.PEDW_ASTHMA_ELECT;
		 ...
		 WHEN MATCHED THEN	UPDATE SET A.PEDW_ASTHMA_MATRN = B.PEDW_ASTHMA_MATRN;
		 ...
		 WHEN MATCHED THEN	UPDATE SET A.PEDW_ASTHMA_OTHER = B.PEDW_ASTHMA_OTHER;

...
print_prevalence = function() {
  ASTHMA_DX_CURRENT.xtab = paste0("
  	...
  	") %>% SQL2dt %>% filter(!WIMDQ %>% is.na) %>% setDT
  ...
  prev.tables[['ASTHMA_DX_CURRENT']] = rbind(
    (
      merge(
        ASTHMA_DX_CURRENT.xtab[ASTHMA_DX_CURRENT == 1, list(n = sum(N)),by = WIMDQ],
        ASTHMA_DX_CURRENT.xtab[                      , list(N = sum(N)),by = WIMDQ],
        by = 'WIMDQ'
      )
    )[, prev := n/N]
    ,
  ...
  prev.tables[['ASTHMA_DX_RX']] = rbind(
    (
      merge(
        ASTHMA_DX_RX.xtab[ASTHMA_DX_RX == 1, list(n = sum(N)),by = WIMDQ],
        ASTHMA_DX_RX.xtab[                 , list(N = sum(N)),by = WIMDQ],
        by = 'WIMDQ'
        )
    )[, prev := n/N]
  	,
  ...
  set_colnames(c('', 'WIMDQ' %>% paste(1:5), 'all'))
